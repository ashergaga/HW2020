...
unordered_map<int, int> vertex_visit, dfn, low;
...
void load_data(string path)
{
	...
	while (infile) 
	{
		...
		if (vertex_visit.empty() || vertex_iter == vertex_visit.end())
		{
			...
			dfn.insert(pair<int, int>(v, 0));
			low.insert(pair<int, int>(v, 0));
		}
	}
	infile.close();
}
...
取2：节点遍历完毕
=======================================================================*/
void DFS(stack<int>& cur_path, int v, int d)
{
	if (d > 7) return;
	cur_path.push(v);
	...
	{
		//边(v, i)存在，但节点i不在当前路径
		if (vertex_visit[edge_mapping[v][i]] == 0)
		{
			DFS(cur_path, edge_mapping[v][i], d + 1); //路径延伸，继续递归
		}
...
void find_circle()
{
	...
	{
		stack<int> cur_path;
		DFS(cur_path, vertex_iter->first, 1);
		vertex_visit[vertex_iter->first] = 2;
	}
}
